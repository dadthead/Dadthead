...
const SeoSingle = () => {
  ...
  const { id } = useParams();
  const seoData = Seo.find((item) => item.id === Number(id));
  ...
  "Boost visibility. Drive traffic. Grow smarter — with <span className="text-amber-400 font-semibold">DAD – The Ad</span>."
</motion.p>
    ...
    <Particles />
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        <h3 className="text-3xl md:text-4xl font-bold mb-6">
          What is {seoData.title}?
        </h3>
        <p className="text-gray-200 mb-6 whitespace-pre-line">{seoData.content}</p>

        <div className="mt-8 space-y-4">
          <p className="text-gray-400">👉 {seoData.point1}</p>
          <p className="text-gray-400">👉 {seoData.point2}</p>
          <p className="text-gray-400">👉 {seoData.point3}</p>
          <p className="text-gray-400">👉 {seoData.point4}</p>
          <p className="text-gray-400">👉 {seoData.point5}</p>
          <p className="text-gray-400">👉 {seoData.point6}</p>
        </div>
      </div>
      ...
      <div className="bg-black text-white py-16 px-4">
        ...
        <div className="max-w-6xl mx-auto flex flex-col gap-16">
          {seoData.whychooseItems.map((item, index) => (
            <div
              key={index}
