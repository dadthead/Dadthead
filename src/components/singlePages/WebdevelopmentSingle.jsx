...
const WebdevelopmentSingle = () => {
      const heroRef = useRef(null);
  
  const { id } = useParams();
  const webData = Webdevelopment.find((item) => item.id === Number(id));
     ...
     className="text-amber-400 ">
      ...
      <div className="relative w-full h-[250px] md:h-[400px]">
        ...
        className="text-white text-center text-lg z-[2] md:text-lg mt-4 px-6 font-light max-w-2xl mx-auto ">
          "At DAD – The Ad, your first click leads to lasting impact."
    ...
    <Particles />
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        <h3 className="text-3xl md:text-4xl font-bold mb-6">
          What is {webData.title} development?
        </h3>
        <p className="text-gray-200 mb-6 whitespace-pre-line">{webData.content}</p>

        <div className="mt-8 space-y-4">
          <p className="text-gray-400">👉 {webData.point1}</p>
          <p className="text-gray-400">👉 {webData.point2}</p>
          <p className="text-gray-400">👉 {webData.point3}</p>
          <p className="text-gray-400">👉 {webData.point4}</p>
          <p className="text-gray-400">👉 {webData.point5}</p>
          <p className="text-gray-400">👉 {webData.point6}</p>
        </div>
      </div>
      ...
      <div className="bg-black text-white py-16 px-4">
        ...
        <div className="max-w-6xl mx-auto flex flex-col gap-16">
          {webData.whychooseItems.map((item, index) => (
            <div
              key={index}
            ...
            >
              ...
              >
                <h3 className="text-xl text-amber-400 md:text-2xl font-semibold mb-3">
                  {item.subtitle}
                </h3>
...
export default WebdevelopmentSingle;
